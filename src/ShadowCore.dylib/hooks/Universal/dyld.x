...
// struct exists: the old memory-level preference gate left the real arrays
...
void shdw_universal_dyld_updatelibs(const struct mach_header* mh, intptr_t vmaddr_slide) {
    if(!mh) {
        return;
    }
...
void shdw_universal_dyld_updatelibs_r(const struct mach_header* mh, intptr_t vmaddr_slide) {
    if(!mh) {
        return;
    }
...
static int replaced_dladdr(const void* addr, Dl_info* info) {
    ...
    const void* remapped = SHDWRemapDladdrAddress(addr, __builtin_return_address(0));
    if(remapped) {
        return original_dladdr(remapped, info);
    }
...
// Unmapped fan-out, driven from the universal remove-image callback — only ever
...
// Reachable via the AR2 emergency kill-switch (Universal_MemoryLevelHiding pref off):
...
static void shadowhook_dyld_rebuild_dyldinfo(void) {
    ...
    // AR2 emergency kill-switch (Universal_MemoryLevelHiding pref off): restore dyld's
...
void shdw_universal_rebind_image(SHDWHookSession* hooks, const void* imageHeader) {
    ...
    if(original_dyld_get_image_name)
        [hooks hookRebindSymbol:@"_dyld_get_image_name" withReplacement:replaced_dyld_get_image_name outOldPtr:NULL inCallerImage:imageHeader];
    if(original_dyld_image_count)
        [hooks hookRebindSymbol:@"_dyld_image_count" withReplacement:replaced_dyld_image_count outOldPtr:NULL inCallerImage:imageHeader];
    if(original_dyld_get_image_header)
        [hooks hookRebindSymbol:@"_dyld_get_image_header" withReplacement:replaced_dyld_get_image_header outOldPtr:NULL inCallerImage:imageHeader];
    if(original_dyld_get_image_vmaddr_slide)
        [hooks hookRebindSymbol:@"_dyld_get_image_vmaddr_slide" withReplacement:replaced_dyld_get_image_vmaddr_slide outOldPtr:NULL inCallerImage:imageHeader];
    if(original_dlsym)
        [hooks hookRebindSymbol:@"dlsym" withReplacement:replaced_dlsym outOldPtr:NULL inCallerImage:imageHeader];
}
...
void shdw_universal_dyld(SHDWHookSession* hooks) {
    ...
    _dyld_register_func_for_add_image(shdw_universal_dyld_updatelibs);
    _shdw_dyld_replay_in_progress = NO;
    _dyld_register_func_for_remove_image(shdw_universal_dyld_updatelibs_r);
...
void shdw_universal_dynamic_libraries_extra(SHDWHookSession* hooks) {
    ...
    // fire the dyld add-image callback on this
